	...
	)//mode: 0:VALID, 1:SAME
{
	...
	CHout:
	for(int cout=0;cout<CHout;cout++) //conv core number
		Hout:
		for(int i=0;i<Hout;i++)       //feature picture Height
			Wout:
			for(int j=0;j<Wout;j++)	  //feature picture Width
			{
				Dtype_acc sum=0;
				Ky:
				for(int ii=0;ii<Ky;ii++)	//
					...
					for(int jj=0;jj<Kx;jj++) //
					{
						...
						{
							CHin:
							for(int cin=0;cin<CHin;cin++) //feature picture number
							{
								...
								std::cout<<"ii:"<<ii<<",jj:"<<jj<<",cin:"<<cin<<",cout:"<<cout<<"\n";
